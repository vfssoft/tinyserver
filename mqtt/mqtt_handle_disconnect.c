...
int tm_mqtt_conn__process_disconnect(ts_t* server, ts_conn_t* c) {
  ...
  if (conn->will) {
    LOG_VERB("[%s] Client is disconnected gracefully, discard the Will message silently", conn->session->client_id);
...
int tm_mqtt_conn__process_tcp_disconnect(ts_t* server, ts_conn_t* c) {
  ...
  LOG_DEBUG("[%s] Disconnected", conn->session->client_id);
  ...
  if (conn->will) {
    LOG_VERB("[%s] Client is disconnected abnormally, publish the Will message silently", conn->session->client_id);
  ...
  if (conn->session->clean_session) {
    LOG_VERB("[%s] Clean session is set, discard the session state", conn->session->client_id);
